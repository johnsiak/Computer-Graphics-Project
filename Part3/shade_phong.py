    ...
    for p1 in edge1:
        if p1 not in vertices:
            if abs(vertices[0][0] - vertices[1][0]) > abs(vertices[0][1] - vertices[1][1]):
                # x axis interpolation for normal and color vectors
                N = interpolate_vectors(vertices[0], vertices[1], vertsn[0], vertsn[1],  p1[0], 1)
                C = interpolate_vectors(vertices[0], vertices[1], vertsc[0], vertsc[1], p1[0], 1)
                updatedcanvas[p1[1]][p1[0]] = light(bcoords, N, C, cam_pos, mat, lights, light_amb)
            else:
                # y axis interpolation for normal and color vectors
                N = interpolate_vectors(vertices[0], vertices[1], vertsn[0], vertsn[1],  p1[1], 2)
                C = interpolate_vectors(vertices[0], vertices[1], vertsc[0], vertsc[1], p1[1], 2)
                updatedcanvas[p1[1]][p1[0]] = light(bcoords, N, C, cam_pos, mat, lights, light_amb)
        else:
            # The current pixel is a vertex, so we use the corresponding color value to calculate the lighting
            index = vertices.index(p1)
            updatedcanvas[p1[1]][p1[0]] = light(bcoords, vertsn[index], vertsc[index], cam_pos,  mat, lights, light_amb)

    for p2 in edge2:
        if p2 not in vertices:
            if abs(vertices[1][0] - vertices[2][0]) > abs(vertices[1][1] - vertices[2][1]):
                # x axis interpolation for normal and color vectors
                N = interpolate_vectors(vertices[1], vertices[2], vertsn[1], vertsn[2],  p2[0], 1)
                C = interpolate_vectors(vertices[1], vertices[2], vertsc[1], vertsc[2], p2[0], 1)
                updatedcanvas[p2[1]][p2[0]] = light(bcoords, N, C, cam_pos, mat, lights, light_amb)
            else:
                # y axis interpolation for normal and color vectors
                N = interpolate_vectors(vertices[1], vertices[2], vertsn[1], vertsn[2],  p2[1], 2)
                C = interpolate_vectors(vertices[1], vertices[2], vertsc[1], vertsc[2], p2[1], 2)
                updatedcanvas[p2[1]][p2[0]] = light(bcoords, N, C, cam_pos, mat, lights, light_amb)
        else:
            # The current pixel is a vertex, so we use the corresponding color value to calculate the lighting
            index = vertices.index(p2)
            updatedcanvas[p2[1]][p2[0]] = light(bcoords, vertsn[index], vertsc[index], cam_pos,  mat, lights, light_amb)

    for p3 in edge3:
        if p3 not in vertices:
            if abs(vertices[1][0] - vertices[2][0]) > abs(vertices[1][1] - vertices[2][1]):
                # x axis interpolation for normal and color vectors
                N = interpolate_vectors(vertices[2], vertices[0], vertsn[2], vertsn[0],  p3[0], 1)
                C = interpolate_vectors(vertices[2], vertices[0], vertsc[2], vertsc[0], p3[0], 1)
                updatedcanvas[p3[1]][p3[0]] = light(bcoords, N, C, cam_pos, mat, lights, light_amb)
            else:
                # y axis interpolation for normal and color vectors
                N = interpolate_vectors(vertices[2], vertices[0], vertsn[2], vertsn[0],  p3[1], 2)
                C = interpolate_vectors(vertices[2], vertices[0], vertsc[2], vertsc[0], p3[1], 2)
                updatedcanvas[p3[1]][p3[0]] = light(bcoords, N, C, cam_pos, mat, lights, light_amb)
        else:
            # The current pixel is a vertex, so we use the corresponding color value to calculate the lighting
            index = vertices.index(p3)
            updatedcanvas[p3[1]][p3[0]] = light(bcoords, vertsn[index], vertsc[index], cam_pos,  mat, lights, light_amb)
